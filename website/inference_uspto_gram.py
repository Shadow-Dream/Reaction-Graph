...
class MPNN(Module):
    ...
    def forward(self, inputs):
        ...
        num_atoms_batch = molecule_aggregation_graph.batch_num_nodes()
        ...
        reaction_features = segment.segment_reduce(num_atoms_batch, node_aggregation, reducer="sum")
    ...
    for reactant_atom in reactants.GetAtoms():
        ...
        for product_atom in products.GetAtoms():
            if product_atom.GetAtomMapNum() == map_number:
                ...
                break
    return reactant_map_index,product_map_index
...
def get_atom_attribute(mol):
    ...
    atom_feature2 = np.eye(len(charge_list), dtype = bool)[[charge_list.index(a.GetFormalCharge()) for a in mol.GetAtoms()]]
    atom_feature3 = np.eye(len(degree_list), dtype = bool)[[degree_list.index(a.GetDegree()) for a in mol.GetAtoms()]]
    atom_feature4 = np.eye(len(hybridization_list), dtype = bool)[[hybridization_list.index(str(a.GetHybridization())) for a in mol.GetAtoms()]]
    atom_feature5 = np.eye(len(hydrogen_list), dtype = bool)[[hydrogen_list.index(a.GetTotalNumHs(includeNeighbors = True)) for a in mol.GetAtoms()]]
    atom_feature6 = np.eye(len(valence_list), dtype = bool)[[valence_list.index(a.GetTotalValence()) for a in mol.GetAtoms()]]
...
def get_reaction_graph(smiles = ""):
    ...
    reactant_list = [Chem.MolFromSmiles(reactant_smiles) for reactant_smiles in reactants_smiles.split(".")]
    product_list = [Chem.MolFromSmiles(product_smiles) for product_smiles in products_smiles.split(".")]

    reactant_map_index,product_map_index = get_reaction_mapping(reactants,products)
    
    num_atom_nodes = reactants.GetNumAtoms() + products.GetNumAtoms()
    num_molecule_nodes = len(reactant_list) + len(product_list)
    ...
    node_type[num_atom_nodes:num_atom_nodes + len(reactant_list),2] = 1
    node_type[num_atom_nodes + len(reactant_list):,3] = 1
    ...
    offset = 0
    edges = []
    for index,molecule in enumerate(list(reactant_list)):
    ...
    edges = []
    for index,molecule in enumerate(list(product_list)):
        edges += [(i + offset,num_atom_nodes + len(reactant_list) + index) for i in range(molecule.GetNumAtoms())]
        offset += molecule.GetNumAtoms()
    edges = np.array(edges)
    aggregate_edge_attribute = np.zeros_like(edges)
    disperse_edge_attribute = np.zeros_like(edges)
    ...
    forward_edges = []
    backward_edges = []
    for reactant_molecule_index in range(len(reactant_list)):
        for product_molecule_index in range(len(product_list)):
            forward_edges.append((reactant_molecule_index + num_atom_nodes, product_molecule_index + num_atom_nodes + len(reactant_list)))
            backward_edges.append((product_molecule_index + num_atom_nodes + len(reactant_list), reactant_molecule_index + num_atom_nodes))
    edges = forward_edges + backward_edges
    for reactant_molecule_index_1 in range(len(reactant_list)):
        for reactant_molecule_index_2 in range(len(reactant_list)):
    ...
    for product_molecule_index_1 in range(len(product_list)):
        for product_molecule_index_2 in range(len(product_list)):
            if product_molecule_index_1 != product_molecule_index_2:
                edges.append((product_molecule_index_1 + num_atom_nodes + len(reactant_list),product_molecule_index_2+ num_atom_nodes + len(reactant_list)))
    ...
    edge_attribute[2*len(forward_edges):2*len(forward_edges) + len(reactant_list)**2 - len(reactant_list),2] = 1
    edge_attribute[2*len(forward_edges) + len(reactant_list)**2 - len(reactant_list):,3] = 1
    edges = np.array(edges)
    src = edges[:,0]
    dst = edges[:,1]
...
parser.add_argument("--dataset_dir", type = str, default = "/root/reaction-graph-backend/Exp_Yield/dataset/reaction_graph")
...
checkpoint = torch.load("uspto_gram.pth")
...
with open(f"{workdir}/uspto_gram.json","w") as f:
    json.dump(result,f)
